...
$("#previewForm").click(function(){
	var s=$('#previewForm').serialize();
	alert("ss=="+s);
	$.ajax({
		cache:true,
		type:"POST",url:'resume/insertResume',
		data:s,
		async:false,
		error:function(){
			alert("简历投递失败！！！");
...
function shouInfo(number){
	...
	$.post("tkfuser/findUsResume",params,function(data){
		...
		$(".expectShow").html('<h5 name="hj_city">'+data[0].hj_city+'</h5>,<h5 name="hj_name">'+data[0].hj_name+',月薪<h5 name="hj_min_salary">'+data[0].hj_min_salary+'-'+data[0].hj_max_salary+','+(data[0].hj_nature=='-0'?'全职':(data[0].hj_nature=='-1'?'兼职':'实习')));
	},"json");
