function scoreChange(a){
	...
	setTimeout(e,300)}function changeAllIds(a){
		var c,d,e,f,b=JSON.parse(a);$("#resumeId").val(b.resumeId.n),
		c=b.expId,d=b.eduId,e=b.projectId,f=b.showId,
		$("#workExperience .wlist li").each(function(){
			...
			$("#projectExperience .plist li").each(function(){
				...
				$("#educationalBackground .elist li").each(function(){
					...
					$("#worksShow .slist li").each(function(){var b,a=$(this).attr("data-id");
					for(b=0;b<f.length;b++)a==f[b].o&&$(this).attr("data-id",f[b].n)})
	}
	function openYear(a){
		$(".profile_select_normal").removeClass("select_focus"),
		$(".boxUpDown").hide(),$(".select_"+a).addClass("select_focus"),
		$(".box_"+a).show()
	}
	...
	function suggestCompany(a,b){
		var c="";
		$.ajax({type:"POST",async:!1,url:ctx+"/c/companySearchCompanyLs.json",data:{kd:a},dataType:"json"}).done(function(a){c=a}),$(b).autocomplete({source:c})}
	function suggestPosition(a,b){
		var c="";
		$.ajax({type:"POST",async:!1,url:ctx+"/jobs/listLs",data:{pl:a},dataType:"json"}).done(function(a){a.success&&(c=a.content.result)}),$(b).autocomplete({source:c})}
	function click_scroll(a){
		var b=$("#"+a).offset();
		switch(a){
		case"expectJob":$("#expectJob .expectAdd").trigger("click");
		break;
		case"workExperience":$("#workExperience .experienceAdd").trigger("click");
		break;
		case"projectExperience":$("#projectExperience .projectAdd").trigger("click");
		break;
		case"educationalBackground":$("#educationalBackground .educationalAdd").trigger("click");
		break;
		case"selfDescription":$("#selfDescription .descriptionAdd").trigger("click");break;
		case"worksShow":$("#worksShow .workAdd").trigger("click");break;
		default:return}
		$("body,html").animate({scrollTop:b.top},400,function(){$("#"+a).animate({borderColor:"#e46a4a"},300,function(){$(this).animate({borderColor:"transparent"})})})
	}

	function img_check(a,b,c){
		var a=$("#"+c);this.AllowExt=".jpg,.gif,.jpeg,.png,.pjpeg",
		this.FileExt=a.val().substr(a.val().lastIndexOf(".")).toLowerCase(),0!=this.AllowExt&&-1==this.AllowExt.indexOf(this.FileExt)?errorTips("只支持jpg、gif、png、jpeg格式，请重新上传","上传头像"):($("#"+c+"_error").text("").hide(),
				$.ajaxFileUpload({url:b,secureuri:!1,fileElementId:c,data:{headPic:a.val()},dataType:"json",
					success:function(a){
						if(a.success){var b=a.content;$("#portraitShow img").attr("src",ctx+"/"+b),$("#basicInfo .basicShow img").attr("src",ctx+"/"+b),$("#portraitNo").hide(),$("#portraitShow").show()}
						else errorTips("上传失败，请重新上传","上传头像")},error:function(){errorTips("上传失败，请重新上传","上传头像")}}))
	}
	function file_check(obj,action_url,id){
		var userId;$("#loadingImg").css("visibility","visible"),
		obj=$("#"+id),userId=$("#userid").val(),
		this.AllowExt=".doc,.docx,.pdf,.ppt,.pptx,.txt,.wps",
		this.FileExt=obj.val().substr(obj.val().lastIndexOf(".")).toLowerCase(),0!=this.AllowExt&&-1==this.AllowExt.indexOf(this.FileExt)?(errorTips("只支持word、pdf、ppt、txt、wps格式文件，请重新上传"),
				$("#loadingImg").css("visibility","hidden")):$.ajaxFileUpload({type:"POST",url:action_url,secureuri:!1,
					fileElementId:id,data:{userId:userId},dataType:"text",
					success:function(jsonStr){
						var html,json=eval("("+jsonStr+")");
						$("#loadingImg").css("visibility","hidden"),
						json.success?(html='<a href="'+ctx+'/nearBy/downloadResume" class="resume_download" title="下载'+json.content.nearbyName+'">'+json.content.nearbyName+"</a>"+'<a  class="fr del" href="javascript:;">删除</a><br /><span class="c9">上传时间：'+json.content.time+"</span>",$("#myResume .resumeUploadDiv").html(html),
								$("#myResume h2 a").html("重新上传"),$.colorbox({inline:!0,href:$("div#uploadFileSuccess"),title:"上传附件简历"})):-1==json.code?$.colorbox({inline:!0,href:$("div#fileResumeUpload"),title:"附件简历上传失败"}):-2==json.code?$.colorbox({inline:!0,href:$("div#fileResumeUploadSize"),
									title:"附件简历上传失败"}):(errorTips("简历上传失败，请重新上传"),$("#loadingImg").css("visibility","hidden"))},error:function(){errorTips("简历上传失败，请重新上传"),
										$("#loadingImg").css("visibility","hidden")}})}!function(a,b){
											function c(a,b,c){var d=k[b.type]||{};return null==a?c||!b.def?null:b.def:(a=d.floor?~~a:parseFloat(a),isNaN(a)?b.def:d.mod?(a+d.mod)%d.mod:0>a?0:d.max<a?d.max:a)}function d(b){var c=i(),d=c._rgba=[];return b=b.toLowerCase(),o(h,function(a,e)
													{var f,g=e.re.exec(b),h=g&&e.parse(g),i=e.space||"rgba";return h?(f=c[i](h),c[j[i].cache]=f[j[i].cache],
															d=c._rgba=f._rgba,!1):void 0}),
															...
															}),$(".select_companyYearEnd").bind("click",function(a){a.stopPropagation(),openYear("companyYearEnd")}),
															$(".box_companyYearEnd").on("click","ul li",function(a){var b,c,d;a.stopPropagation(),b=$(this).text(),

															c=$(this).parents(".experienceForm").find(".companyMonthEnd").val(),selectYear("companyYearEnd",b),
															d=$(this).parents(".experienceForm"),"至今"==$.trim(b)?(d.find(".companyMonthEnd").val("至今"),
																	d.find(".select_companyMonthEnd").css("color","#333").val("至今"),
																	d.find(".select_companyMonthEnd").unbind("click")):("至今"==$.trim(c)&&(
																			d.find(".companyMonthEnd").val(""),d.find(".select_companyMonthEnd").css("color","#999").val("结束时间")),
																			d.find(".select_companyMonthEnd").bind("click",function(a){a.stopPropagation(),openYear("companyMonthEnd")})),
																			...
																			$(".select_companyMonthEnd").bind("click",function(a){a.stopPropagation(),
																				openYear("companyMonthEnd")}),$(".box_companyMonthEnd").on("click","ul li",function(a){var b,c;a.stopPropagation(),b=$(this).text(),selectYear("companyMonthEnd",b),
																				c=$(this).parents(".experienceForm"),$(this).parents(".box_companyMonthEnd").siblings(".companyMonthEnd").hasClass("error")&&$(this).parents(".experienceForm").validate().element(c.find(".companyMonthEnd")),
																				...
																				c={obj:$("#workExperience"),companyNameVal:"",positionNameVal:"",companyYearStartVal:"",companyMonthStartVal:"",
																companyYearEndVal:"",companyMonthEndVal:"",
																Add:function(){this.obj.children(".experienceEdit").find(".companyName").val(""),
																	...
																	this.obj.children(".experienceEdit").find(".select_companyMonthEnd").css("color","#999").val("结束月份"),placeholderFn(),
																	this.obj.children(".experienceAdd").addClass("dn"),
																	this.obj.children(".experienceShow").addClass("dn"),
																	this.obj.children(".experienceEdit").removeClass("dn"),j(this.obj)},
																	AddMore:function(){
																		...
																		this.obj.children(".experienceShow").find(".expId").val(""),placeholderFn(),
																		this.obj.children(".experienceShow").children(".experienceForm").show(),j(this.obj)},
																		Edit:function(a){
																			...
																			this.obj.children(".c_add").addClass("dn"),j(this.obj)},
																			Del:function(a){
																				...
																				$.ajax({
																					...
																					data:{id:b}
																						...
																						function(b){
																							b.success?(a.parent("li").remove(),
																									""==b.content.workExperiences||null==b.content.workExperiences?(c.obj.children(".experienceShow").addClass("dn"),
																											c.obj.children(".experienceEdit").addClass("dn"),
																											c.obj.children(".experienceAdd").removeClass("dn"),
																											c.obj.children(".c_add").addClass("dn")):c.obj.children(".experienceShow").children(".experienceForm").hide(),
																																...
																																c.obj.on("click",".sm_edit",function(){
																																	...
																																	c.obj.on("click",".sm_del",function(){
																																		...
																																		c.obj.children(".experienceShow").on({
																																			...
																																			c.obj.children(".experienceEdit").find(".btn_profile_cancel").bind("click",function(){c.AddCancel()}),
																																			c.obj.children(".experienceShow").find(".btn_profile_cancel").bind("click",function(){c.AddMoreCancel(),
																																				c.obj.find(".c_add").removeClass("dn")}),c.obj.delegate(".companyName","keyup",function(){
																																					var a=$.trim($(this).val());
																																					...
																																					c.obj.delegate(".positionName","keyup",function(){
																																						...
																																						$(".experienceForm").each(function(){
																																							$(this).validate({
																																								groups:{companyStart:"companyYearStart companyMonthStart",companyEnd:"companyYearEnd companyMonthEnd"},
																																								rules:{companyName:{required:!0,maxlenStr:100},positionName:{required:!0,checkNum:!0,maxlenStr:100},
																																									companyYearStart:{required:!0,checkWorkYear:!0},companyMonthStart:{required:!0,checkWorkYear:!0},
																																									companyYearEnd:{required:!0,checkWorkYear:!0},companyMonthEnd:{required:function(){
																																										return"至今"==$(".companyYearEnd").val()?!1:!0},
																																										checkWorkYear:!0}},
																																										messages:{
																																											companyName:{
																																												required:"请输入有效的公司名称，如：拉勾网",maxlenStr:"请输入有效的公司名称，如：拉勾网"},
																																												positionName:{
																																													required:"请输入有效的职位名称，如：产品经理",checkNum:"请输入有效的职位名称，如：产品经理",maxlenStr:"请输入有效的职位名称，如：产品经理"},
																																													companyYearStart:{
																																														required:"请选择开始时间",checkWorkYear:"开始时间需早于结束时间"},
																																														companyMonthStart:{
																																															required:"请选择开始时间",checkWorkYear:"开始时间需早于结束时间"},
																																															companyYearEnd:{
																																																required:"请选择结束时间",checkWorkYear:"结束时间需晚于开始时间"},
																																																companyMonthEnd:{
																																																	required:"请选择结束时间",checkWorkYear:"结束时间需晚于开始时间"}
																																										},
																																										errorPlacement:function(a,b){
																																											...
																																											"companyMonthEnd"==b.attr("name")?
																																													...
																																													submitHandler:function(a){
																																														...
																																														l=$("#resubmitToken").val();$(a).find(":submit").val("保存中...").attr("disabled",!0),
																																														$.ajax({
																																																...
																																																function(b){
																																																	...
																																																	if($("#resubmitToken").val(b.resubmitToken),b.success){
																																																		for(d=b.content.workExperiences,e="",f=0;f<d.length;f++)
																																																			...
																																																			$("#resumeScore .which span").attr("rel",b.content.infoCompleteStatus.nextStage),
																																																			scoreChange(b.content.infoCompleteStatus.score),
																																																			k(c.obj),b.content.isNew&&changeAllIds(b.content.jsonIds)
																																																	}else alert(b.msg);
																																																	$(a).find(":submit").val("保 存").attr("disabled",!1)})}})}),
																																																	$("#projectExperience").on("click",".select_projectYearStart",
																																																			function(a){a.stopPropagation(),
																																																		$(".profile_select_normal").removeClass("select_focus"),
																																																		$(".boxUpDown").hide(),$(this).addClass("select_focus"),
																																																		$(this).siblings(".box_projectYearStart").show()}),
																																																		$("#projectExperience").on("click",".box_projectYearStart li",
																																																				function(a){
																																																			var b,c;
																																																			a.stopPropagation(),
																																																			b=$(this).text(),
																																																			...
																																																			$("#projectExperience").on("click",".select_projectMonthStart",
																																																					function(a){a.stopPropagation(),
																																																				$(".profile_select_normal").removeClass("select_focus"),
																																																				$(".boxUpDown").hide(),
																																																				$(this).addClass("select_focus"),
																																																				$(this).siblings(".box_projectMonthStart").show()}),
																																																				$("#projectExperience").on("click",".box_projectMonthStart li",
																																																						function(a){
																																																					var b,c;
																																																					a.stopPropagation(),
																																																					b=$(this).text(),
																																																					...
																																																					c=$(this).parents(".projectForm"),
																																																					$(this).parents(".box_projectMonthStart").siblings(".projectMonthStart").hasClass("error")&&$(this).parents(".projectForm").validate().element(c.find(".projectMonthStart")),
																																																					c.find(".projectYearEnd").hasClass("error")?$(this).parents(".projectForm").validate().element(c.find(".projectYearEnd")):c.find(".projectMonthEnd").hasClass("error")&&$(this).parents(".projectForm").validate().element(c.find(".projectMonthEnd"))}),
																																																					$("#projectExperience").on("click",".select_projectYearEnd",
																																																							function(a){a.stopPropagation(),
																																																						$(".profile_select_normal").removeClass("select_focus"),
																																																						$(".boxUpDown").hide(),$(this).addClass("select_focus"),
																																																						$(this).siblings(".box_projectYearEnd").show()}),
																																																						$("#projectExperience").on("click",".box_projectYearEnd li",
																																																								...
																																																								d=$(this).parents(".projectForm"),"至今"==$.trim(b)?(d.find(".projectMonthEnd").val("至今"),
																																																										d.find(".select_projectMonthEnd").css("color","#333").val("至今"),
																																																										$("#projectExperience").off("click",".select_projectMonthEnd")):("至今"==$.trim(c)&&(d.find(".projectMonthEnd").val(""),
																																																												d.find(".select_projectMonthEnd").css("color","#999").val("结束时间")),
																																																												$("#projectExperience").on("click",".select_projectMonthEnd",
																																																														function(a){a.stopPropagation(),
																																																													$(".profile_select_normal").removeClass("select_focus"),
																																																													$(".boxUpDown").hide(),
																																																													$(this).addClass("select_focus"),
																																																													...
																																																													d.find(".projectYearStart").hasClass("error")?$(this).parents(".projectForm").validate().element(d.find(".projectYearStart")):d.find(".projectMonthStart").hasClass("error")&&$(this).parents(".projectForm").validate().element(d.find(".projectMonthStart"))}),$("#projectExperience").on("click",".select_projectMonthEnd",function(a){a.stopPropagation(),$(".profile_select_normal").removeClass("select_focus"),$(".boxUpDown").hide(),$(this).addClass("select_focus"),$(this).siblings(".box_projectMonthEnd").show()}),$("#projectExperience").on("click",".box_projectMonthEnd li",
																																																															function(a){
																																																														var b,c;
																																																														a.stopPropagation(),
																																																														b=$(this).text(),
																																																														$(this).parents(".box_projectMonthEnd").siblings(".select_projectMonthEnd").val(b).css("color","#333").removeClass("select_focus"),
																																																														$(this).parents(".box_projectMonthEnd").siblings(".projectMonthEnd").val(b),
																...
																resetForm:function(){
																	...
																	Add:function(){this.resetForm(),placeholderFn(),this.obj.children(".projectAdd").addClass("dn"),
																		this.obj.children(".projectShow").addClass("dn"),this.obj.children(".projectEdit").removeClass("dn"),j(this.obj)},
																		AddMore:function(){this.resetForm(),placeholderFn(),this.obj.children(".projectShow").prepend(this.obj.children(".projectEdit").children("form").clone()),this.obj.children(".projectShow").children("form").css("borderBottom","1px solid #e5e5e5").show(),this.obj.children(".projectEdit").addClass("dn"),e(),j(this.obj)},Edit:function(a){var m,b=a.parents("li"),c=b.attr("data-id"),d=b.find("div.f16").attr("data-proName"),
																			f=b.find("div.f16").attr("data-posName"),g=b.find("div.f16").attr("data-startY"),
																			h=b.find("div.f16").attr("data-startM"),i=b.find("div.f16").attr("data-endY"),k=b.find("div.f16").attr("data-endM"),
																			l=b.find(".dl1").html();l&&(l=$.trim(l.replace(/<br>/gi,""))),this.resetForm(),m=this.obj.children(".projectEdit").children("form").clone(),
																			m.find(".projectName").val(d),m.find(".thePost").val(f),""!=g&&(m.find(".projectYearStart").val(g),
																					m.find(".select_projectYearStart").css("color","#333").val(g)),""!=h&&(m.find(".projectMonthStart").val(h),
																							...
																							"至今"==$.trim(k)&&m.find(".select_projectMonthEnd").unbind("click"),m.find(".projectDescription").val(l),
																							m.find(".word_count").children("span").text(500-m.find(".projectDescription").val().length),
																							m.find(".projectId").val(c),a.parents("li").prepend(m),
																							placeholderFn(),a.parents("li").children(".projectList").addClass("dn"),e(),j(this.obj)},
																							Del:function(a){var b=a.parents("li").attr("data-id");
																							$.ajax({url:ctx+"/projectExperience/delProject.json",type:"POST",
																								data:{id:b}}).done(function(b){b.success?(a.parents("li").remove(),
																										$("ul.plist li:last-child").addClass("noborder"),
																										(""==b.content.projectExperiences||null==b.content.projectExperiences)&&(d.obj.children(".projectShow").addClass("dn"),d.obj.children(".projectEdit").addClass("dn"),d.obj.children(".projectAdd").removeClass("dn"),
																												d.obj.children(".c_add").addClass("dn")),$("#lastChangedTime span").text(b.content.refreshTime),
																												$("#resumeScore .which div").text(b.content.infoCompleteStatus.msg),
																												$("#resumeScore .which span").attr("rel",b.content.infoCompleteStatus.nextStage),scoreChange(b.content.infoCompleteStatus.score),
																												k(d.obj),b.content.isNew&&changeAllIds(b.content.jsonIds)):alert(b.msg)})},
																												AddCancel:function(){this.obj.find(".projectAdd").removeClass("dn"),
																													this.obj.find(".projectShow").addClass("dn"),this.obj.find(".projectEdit").addClass("dn"),k(this.obj)},
																													AddMoreCancel:function(a){a.parents(".projectForm").siblings(".projectList").removeClass("dn"),a.parents(".projectForm").remove(),
																														k(this.obj)}},d.obj.find(".plist li:last-child").addClass("noborder"),
																														d.obj.find(".projectAdd").bind("click",function(){d.Add()}),
																														d.obj.find(".c_add").bind("click",function(){d.AddMore(),$(this).addClass("dn")}),
																														d.obj.on("click",".sm_edit",function(){d.Edit($(this))}),
																														d.obj.on("click",".sm_del",function(){confirm("确认要删除吗？")&&d.Del($(this))}),
																														d.obj.children(".projectShow").on({mouseenter:function(){$(this).children(".plist").find(".sm_del,.sm_edit").removeClass("dn")},
																															mouseleave:function(){$(this).children(".plist").find(".sm_del,.sm_edit").addClass("dn")}}),
																															d.obj.on("keyup",".s_textarea",function(){var a=$(this);$.trim(a.val()).length>500?a.val($.trim(a.val()).substring(0,500)):a.next(".word_count").children("span").text(500-$.trim(a.val()).length)}),
																															d.obj.children(".projectEdit").find(".btn_profile_cancel").bind("click",function(){d.AddCancel()}),
																															d.obj.children(".projectShow").on("click",".btn_profile_cancel",function(){d.AddMoreCancel($(this)),
																																d.obj.find(".c_add").removeClass("dn")}),e(),
																																$(".select_schoolYearStart").bind("click",function(a){a.stopPropagation(),
																																	openYear("schoolYearStart")}),
																																	$(".box_schoolYearStart").on("click","ul li",function(a){a.stopPropagation();
																																	var b=$(this).text();selectYear("schoolYearStart",b),
																																	...
																																	var b=$(this).text();selectYear("schoolYearEnd",b),
																																	$(this).parents(".educationalForm").validate().element($(".schoolYearEnd"))}),
																																	$(".select_degree").bind("click",function(a){a.stopPropagation(),$(this).addClass("select_focus"),
																																		$(".select_schoolYearStart").removeClass("select_focus"),
																																		$(".select_schoolYearEnd").removeClass("select_focus"),
																																		$(".box_schoolYearStart").hide(),$(".box_schoolYearEnd").hide(),
																																		$(".box_degree").show()}),$(".box_degree").on("click","ul li",function(a){a.stopPropagation();
																																		var b=$(this).text();$(".select_degree").val(b).css("color","#333").removeClass("select_focus"),$(".degree").val(b),
																																		$(".box_degree").hide(),$(this).parents(".educationalForm").validate().element($(".degree"))}),
																																		f={obj:$("#educationalBackground"),schoolNameVal:"",degreeVal:"",professionalNameVal:"",schoolYearStartVal:"",schoolYearEndVal:"",
																Add:function(){this.obj.children(".educationalEdit").find(".schoolName").val(""),
																	...
																	this.obj.children(".educationalEdit").find(".select_schoolYearEnd").css("color","#999").val("结束年份"),placeholderFn(),
																	this.obj.children(".educationalAdd").addClass("dn"),this.obj.children(".educationalShow").addClass("dn"),
																	this.obj.children(".educationalEdit").removeClass("dn"),j(this.obj)},
																...
																resetForm:function(){
																	...
																	this.obj.children(".workEdit").find(".wor" +
																	"kDescription").val(""),
																	...
																	Add:function(){
																		...
																		j(this.obj)},
																		AddMore:function(){this.resetForm(),
																			...
																			Edit:function(a){
																				...
																				f.find(".showId").val(c),a.parents("li").prepend(f),
																				placeholderFn(),
																				...
																				Del:function(a){
																					...
																					$.ajax({url:ctx+"/workShow/delws.json",
																						type:"POST",
																						data:{id:b}}).done(function(b){b.success?(a.parents("li").remove(),
																																						...
																																						h.obj.on("keyup",".s_textarea",function(){
																																							...
																																							i(),$("#resumeSet h2 span").bind("click",function(){
																																								...
																																								$.ajax({
																																									...
																																									type:"GET"}).done(
																																											function(a){
																																												a.success?(a.content[0].isDefault&&(b.find(".resume0").click(),
																																														b.children(".defaultResume").val(a.content[0].type)),
																																														a.content[1].isDefault&&(b.find(".resume1").click(),
																																																b.children(".defaultResume").val(a.content[1].type),
																																																a.content[1].perfect||(b.find(".setTip").text("在线简历还未达可投递标准，请完善后选择").show(),
																																																		b.find(".btn_profile_save").attr("disabled","disabled"))),
																																																		a.content[0].perfect||a.content[1].perfect||(b.find(".setTip").text("你还没有可投递的简历").show(),
																																																				b.find(".btn_profile_save").attr("disabled","disabled")),
																																																				b.children(".noSet").hide(),b.children("#resumeSetForm").removeClass("dn"),
																																																				b.children("#resumeSetForm").find('input[type="radio"]').bind("click",
																																																						function(){
																																																					1!=$(this).val()||a.content[1].perfect?0!=$(this).val()||a.content[0].perfect?(b.find(".setTip").text("").hide(),
																																																							b.find(".btn_profile_save").removeAttr("disabled")):(b.find(".setTip").text("你还没有附件线简历，请上传后选择").show(),
																																																									b.find(".btn_profile_save").attr("disabled","disabled")):(b.find(".setTip").text("在线简历还未达可投递标准，请完善后选择").show(),
																																																											b.find(".btn_profile_save").attr("disabled","disabled"))})):alert(a.msg)})}),
																																																											$("#resumeSetForm .btn_profile_cancel").bind("click",
																																																													function(){var b=$(this).parents("#resumeSet"),
																																																												c=b.children(".defaultResume").val();
																																																													-1==c?(b.children(".noSet").show(),
																																																															b.children(".set0").hide(),
																																																															b.children(".set1").hide(),
																																																															b.children("#resumeSetForm").addClass("dn")):(b.children(".noSet").hide(),
																																																																					...
																																																																					errorPlacement:function(a,b){
																																																																						...
																																																																						submitHandler:function(b){
																																																																							...
																																																																							$.ajax({
																																																																								...
																																																																								type:"POST",
																																																																								data:{type:c}}).done(function(d){d.success?(a=!0,
																																																																										$("#resumeSet .noSet").hide(),
																																																																										$("#resumeSet .set"+c).show(),
																																																																										$("#resumeSet .defaultResume").val(c),
																																																																										$("#resumeSetForm").addClass("dn")):alert(d.msg),
																																																																										...
																																																																										$("#myResume").on("click",".resumeUploadDiv a.del",
																																																																											...
																																																																											confirm("你确定要删除该附件吗？删除后你设置的默认投递简历也将失效")&&$.ajax({
																																																																												...
																																																																												type:"GET"}).done(function(a){
																																																																													a.success?($("#myResume h2 a").html("上传简历"),
																																																																															$("#myResume .resumeUploadDiv").html("暂无附件简历")):alert(a.msg)})})}),
